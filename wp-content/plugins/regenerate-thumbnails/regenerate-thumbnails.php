...
Plugin Name:  Regenerate Thumbnails1
...
Author URI:   http://www.viper007bond.com1/
Text Domain:  regenerate-thumbnails1
...
<?php
		...
		if ( ! empty( $_POST['regenerate-thumbnails'] ) || ! empty( $_REQUEST['ids'] ) ) {
			...
			if ( ! empty( $_REQUEST['ids'] ) ) {
				...
				if ( ! $images = $wpdb->get_results( "SELECT p1.ID FROM $wpdb->posts p1 INNER JOIN $wpdb->posts p2 ON p1.post_parent = p2.ID WHERE p1.post_type = 'attachment' AND p1.post_mime_type LIKE 'image/%' AND (p2.post_type='property' OR p2.post_type='news') GROUP BY p1.guid ORDER BY p1.ID DESC" ) ) {
