    ...
    window.send_to_editor = function (html) {
      if (formfield) {
        ...
        if (!fileurl) {
          ...
          if (img_id == '') {
            alert(wds_object.translate.you_must_select_an_audio_file);
            tb_remove();
            return;
          }
          window.parent.document.getElementById(input_id).value = fileurl;
  ...
  for (var i = 0; i < wds_rl_butt_type[type_key].length - 1; i++) {
    ...
    divs += '<div class="spider_option_cont" value="' + i + '"  onclick="change_rl_butt_color(this, ' + type_key + ')" > ' +
			  '<div  class="spider_option_cont_title" >' +
			    + wds_object.translate.color +'-'+ num +
			  '</div>' +
...
function bwg_change_album_view_type(type) {
  if (type == 'thumbnail') {
    jQuery("#album_thumb_dimensions").html(wds_object.translate.album_thumb_dimensions);
	jQuery("#album_thumb_dimensions_x").css('display', '');
	jQuery("#album_thumb_height").css('display', '');
  }
  else {
    jQuery("#album_thumb_dimensions").html(wds_object.translate.album_thumb_width); 
...
function wds_add_layer(type, id, layerID, duplicate, files, edit) {
  ...
  var layer_effects_out = {
    ...
    'hinge' : wds_object.translate.hinge,
    'rubberBand' : wds_object.translate.rubberBand,
    'lightSpeedOut' : wds_object.translate.lightSpeedOut,
    'rollOut' : wds_object.translate.rollOut,
  ...
  for (var i in layer_effects_in) {
    layer_effects_in_option += '<option ' + ((jQuery.inArray(i, free_layer_effects) == -1) ? 'disabled="disabled" title="'+ wds_object.translate.this_functionality_is_disabled_in_free_version +'"' : '') + ' value="' + i + '" ' + (i == laydef_options.default_layer_effect_in ? 'selected' : '') + '>' + layer_effects_in[i] + '</option>';
  }
  for (var i in layer_effects_out) {
    layer_effects_out_option += '<option ' + ((jQuery.inArray(i, free_layer_effects) == -1) ? 'disabled="disabled" title="'+ wds_object.translate.this_functionality_is_disabled_in_free_version +'"' : '') + ' value="' + i + '" ' + (i == laydef_options.default_layer_effect_out ? 'selected' : '') + '>' + layer_effects_out[i] + '</option>';
  }
  ...
  var tbody = '<tr class="wds_layer_head_tr">' +
                '<td colspan="4" class="wds_layer_head">' +
                  ...
                  '<span class="wds_layer_label" onclick="wds_showhide_layer(\'' + tbodyID + '\', 0)"><input id="' + prefix + '_title" name="' + prefix + '_title" type="text" class="wds_layer_title" style="width: 120px; padding:5px; color:#00A2D0; " value="Layer ' + wds_layerID + '" title="'+ wds_object.translate.layer_title +'" /></span></div>' +
              ...
              '</tr>';
  var text = '<td class="spider_label"><label for="' + prefix + '_text">'+ wds_object.translate.text +'</label></td>' +
             '<td><textarea id="' + prefix + '_text" name="' + prefix + '_text" style="width: 222px; height: 60px; resize: vertical;" onkeyup="wds_new_line(\'' + prefix + '\')">'+ wds_object.translate.sample_text +'</textarea></td>';
  var text_dimensions = '<td title="Leave blank to keep the initial width and height." class="wds_tooltip spider_label"><label for="' + prefix + '_image_width">'+ wds_object.translate.dimensions +'</label></td>' +
                        '<td>' +
                          ...
                          '<input id="' + prefix + '_image_scale" type="checkbox" onchange="wds_break_word(this, \'' + prefix + '\')" name="' + prefix + '_image_scale" checked="checked"/><label for="' + prefix + '_image_scale">'+ wds_object.translate.break_word +'</label>' +
                          '</td>';
  var alt = '<td title="'+ wds_object.translate.set_the_html_attribute_specified_in_the_img_tag +'" class="wds_tooltip spider_label"><label for="' + prefix + '_alt">Alt: </label></td>' +
             '<td><input type="text" id="' + prefix + '_alt" name="' + prefix + '_alt" value=""  />' +
                 '</td>';
  var link = '<td title="'+ wds_object.translate.use_http_and_https_for_external_links +'" class="wds_tooltip spider_label"><label for="' + prefix + '_link">'+ wds_object.translate.link +'</label></td>' +
             '<td><input type="text" id="' + prefix + '_link" name="' + prefix + '_link" value=""  />' +
                 '<input id="' + prefix + '_target_attr_layer" type="checkbox"  name="' + prefix + '_target_attr_layer" value="1" checked="checked" /><label for="' + prefix + '_target_attr_layer">'+ wds_object.translate.open_in_a_new_window +'</label>' +
                 '</td>';
  var position = '<td title="'+ wds_object.translate.in_addition_you_can_drag_and_drop_the_layer_to_a_desired_position +'" class="wds_tooltip spider_label"><label>'+ wds_object.translate.position +'</label></td>' +
  ...
  if (type == 'text') {
    position += '<input id="' + prefix + '_align_layer" type="checkbox" name="' + prefix + '_align_layer" value="1"       onchange="wds_position_left_disabled(\'' + prefix + '\');" /><label for="' + prefix + '_align_layer">'+ wds_object.translate.fixed_step_left_center_right+'</label>';
  }                 
    position += '</td>';
  var published = '<td class="spider_label"><label>'+ wds_object.translate.published +'</label></td>' +
                  '<td><input type="radio" id="' + prefix + '_published1" name="' + prefix + '_published" checked="checked" value="1" ><label for="' + prefix + '_published1">'+ wds_object.translate.yes +'</label>' +
                      '<input type="radio" id="' + prefix + '_published0" name="' + prefix + '_published" value="0" /><label for="' + prefix + '_published0">'+ wds_object.translate.no +'</label><div class="spider_description"></div></td>';
  var color = '<td class="spider_label"><label for="' + prefix + '_color">'+ wds_object.translate.color +'</label></td>' +
  ...
  var hover_color_text = '<td class="spider_label"><label for="' + prefix + '_hover_color_text">'+ wds_object.translate.hover_color +'</label></td>' +
  ...
  var size = '<td class="spider_label"><label for="' + prefix + '_size">' + wds_object.translate.font_size + ' </label></td>' +
              '<td>' +
                ...
                '<span style="display:table-cell;">' +
                  ...
                  '<div class="spider_description">' + wds_object.translate.min_size + '</div>' +
                '</span>' +
              '</td>';
  var ffamily = '<td class="spider_label"><label for="' + prefix + '_ffamily">'+ wds_object.translate.font_family +'</label></td>' +
                '<td><select class="select_icon"  style="width: 200px;" name="' + prefix + '_ffamily" id="' + prefix + '_ffamily" onchange="wds_change_fonts(\'' + prefix + '\', 1)"></select>' +
                    '<input type="radio" id="' + prefix + '_google_fonts1" name="' + prefix + '_google_fonts" value="1" onchange="wds_change_fonts(\'' + prefix + '\');" ' +((laydef_options.default_layer_google_fonts == 1) ? "checked='checked'" : "")+ ' /><label for="' + prefix + '_google_fonts1">'+ wds_object.translate.google_fonts +'</label>' +
                    '<input type="radio" id="' + prefix + '_google_fonts0" name="' + prefix + '_google_fonts" ' +((laydef_options.default_layer_google_fonts == 0) ? "checked='checked'" : "")+ ' value="0" onchange="wds_change_fonts(\'' + prefix + '\');" /><label for="' + prefix + '_google_fonts0">'+ wds_object.translate.default +'</label>' +
                    '<div class="spider_description"></div></td>';
  var fweight = '<td class="spider_label"><label for="' + prefix + '_fweight">'+ wds_object.translate.font_weight +'</label></td>' +
  ...
  var padding = '<td title="'+ wds_object.translate.use_css_type_values +'" class="wds_tooltip spider_label"><label for="' + prefix + '_padding">'+ wds_object.translate.padding +'</label></td>' +
  ...
  var fbgcolor = '<td class="spider_label"><label for="' + prefix + '_fbgcolor">'+ wds_object.translate.background_color +'</label></td>' +
  ...
  var fbgtransparent = '<td title="'+ wds_object.translate.value_must_be_between_0_to_100 +'" class="wds_tooltip spider_label"><label for="' + prefix + '_transparent">'+ wds_object.translate.transparent +'</label></td>' +
  ...
  var imgtransparent = '<td title="Value must be between 0 to 100." class="wds_tooltip spider_label"><label for="' + prefix + '_imgtransparent">'+ wds_object.translate.transparent +'</label></td>' +
  ...
  var border_width = '<td class="spider_label"><label for="' + prefix + '_border_width">'+ wds_object.translate.border +'</label></td>' +
  ...
  var border_radius = '<td title="'+ wds_object.translate.use_css_type_values +'" class="wds_tooltip spider_label"><label for="' + prefix + '_border_radius">'+ wds_object.translate.radius +'</label></td>' +
  ...
  var shadow = '<td title="'+ wds_object.translate.use_css_type_values_e_g_10_10_15_888888 +'" class="wds_tooltip spider_label"><label for="' + prefix + '_shadow">Shadow: </label></td>' +
  ...
  var dimensions = '<td class="spider_label"><label>'+ wds_object.translate.dimensions +'</label></td>' +
                   '<td>' +
                     ...
                     '<input type="checkbox" name="' + prefix + '_image_scale" id="' + prefix + '_image_scale" onchange="wds_scale(this, \'' + prefix + '\')" /><label for="' + prefix + '_image_scale">'+ wds_object.translate.scale +'</label>' +
                     '<input class="wds_not_image_buttons_grey wds_free_button" type="button" value="Edit Image" onclick="alert(\''+ wds_object.translate.this_functionality_is_disabled_in_free_version +'\')" />' +
                     '<div class="spider_description">'+ wds_object.translate.set_width_and_height_of_the_image +'</div></td>';
  var social_button = '<td class="spider_label"><label for="' + prefix + '_social_button">'+ wds_object.translate.social_button +'</label></td>' +
  ...
  var transparent = '<td title="Value must be between 0 to 100." class="wds_tooltip spider_label"><label for="' + prefix + '_transparent">'+ wds_object.translate.transparent +'</label></td>' +
  ...
  var hover_color = '<td class="spider_label"><label for="' + prefix + '_hover_color">'+ wds_object.translate.hover_color +'</label></td>' +
  ...
  var layer_effect_in = '<td class="spider_label"><label>'+ wds_object.translate.effect_in +'</label></td>' +
                   '<td>' +
                    '<span style="display: table-cell;">' +
                      '<input type="text" name="' + prefix + '_start" id="' + prefix + '_start" value="' + laydef_options.default_layer_start + '" class="spider_int_input" />'+ wds_object.translate.ms +
                      '<div class="spider_description">'+ wds_object.translate.start +'</div>' +
                    '</span>' +
                    '<span style="display: table-cell;">' +
                      ...
                      '<div class="spider_description">'+ wds_object.translate.effect +'</div>' +
                    '</span>' +
                    '<span style="display: table-cell;">' +
                      '<input id="' + prefix + '_duration_eff_in" class="spider_int_input" type="text"  onkeypress="return spider_check_isnum(event)" onchange="wds_trans_effect_in(\'' + id + '\', \'' + prefix + '\', ' + ((type == 'social') ? 1 : 0) + '); wds_trans_end(\'' + prefix + '\', jQuery(\'#' + prefix + '_layer_effect_in\').val());" value="' + laydef_options.default_layer_duration_eff_in + '" name="' + prefix + '_duration_eff_in">'+ wds_object.translate.ms +
                      '<div class="spider_description">'+ wds_object.translate.duration +'</div>' +
                    '</span>' +
                    '<span style="display: table-cell;">' +
                      '<input id="' + prefix + '_infinite_in" type="text" name="' + prefix + '_infinite_in" value="' + laydef_options.default_layer_infinite_in + '" class="spider_int_input" title="'+ wds_object.translate.for_play_infinte_times +'" ' + (laydef_options.default_layer_effect_in == 'none' ? 'disabled="disabled"' : '') + ' onchange="wds_trans_effect_in(\'' + id + '\', \'' + prefix + '\', ' + ((type == 'social') ? 1 : 0) + '); wds_trans_end(\'' + prefix + '\', jQuery(\'#' + prefix + '_layer_effect_in\').val());" />' +
                      '<div class="spider_description">'+ wds_object.translate.iteration +'</div>' +
                    '</span>' +
                    '<div class="spider_description spider_free_version">'+ wds_object.translate.some_effects_are_disabled_in_free_version +'</div>' +
                    '</td>';
  var layer_effect_out = '<td class="spider_label"><label>'+ wds_object.translate.effect_out +'</label></td>' +
                   '<td>' +
                    '<span style="display: table-cell;">' +
                      '<input type="text" name="' + prefix + '_end" id="' + prefix + '_end" value="' + laydef_options.default_layer_end + '" class="spider_int_input" /> '+ wds_object.translate.ms +
                      '<div class="spider_description">'+ wds_object.translate.start +'</div>' +
                    '</span>' +
                    '<span style="display: table-cell;">' +
                    ...
                    '</span>' +
                    '<span style="display: table-cell;">' +
                      '<input id="' + prefix + '_duration_eff_out" class="spider_int_input" type="text" onkeypress="return spider_check_isnum(event)" onchange="wds_trans_effect_out(\'' + id + '\', \'' + prefix + '\', ' + ((type == 'social') ? 1 : 0) + '); wds_trans_end(\'' + prefix + '\', jQuery(\'#' + prefix + '_layer_effect_out\').val());" value="' + laydef_options.default_layer_duration_eff_out + '" name="' + prefix + '_duration_eff_out"> '+ wds_object.translate.ms +
                      '<div class="spider_description">'+ wds_object.translate.duration +'</div>' +
                    '</span>' +
                    '<span style="display: table-cell;">' +
                    '<input id="' + prefix + '_infinite_out" type="text" name="' + prefix + '_infinite_out" value="' + laydef_options.default_layer_infinite_out + '" class="spider_int_input" title="'+ wds_object.translate.for_play_infinte_times +'" ' + (laydef_options.default_layer_effect_out == 'none' ? 'disabled="disabled"' : '') + ' onchange="wds_trans_effect_out(\'' + id + '\', \'' + prefix + '\', ' + ((type == 'social') ? 1 : 0) + '); wds_trans_end(\'' + prefix + '\', jQuery(\'#' + prefix + '_layer_effect_out\').val());" />' +
                    '<div class="spider_description">'+ wds_object.translate.iteration +'</div>' +
                    '</span>' +
                    '<div class="spider_description spider_free_version">'+ wds_object.translate.some_effects_are_disabled_in_free_version +'</div>' + 
                   '</td>';
  var add_class = '<td class="spider_label"><label for="' + prefix + '_add_class">'+ wds_object.translate.add_class +'</label></td>' +
  ...
  var text_alignment = '<td class="spider_label"><label for="' + prefix + '_text_alignment">'+ wds_object.translate.text_alignment +' </label></td>' +
  ...
  var static_layer = '<td class="spider_label"><label for="' + prefix + '_static_layer">'+ wds_object.translate.static_layer +'</label></td>' + '<td><input id="' + prefix + '_static_layer" type="checkbox" name="' + prefix + '_static_layer" value="1" /><div class="spider_description">'+ wds_object.translate.the_layer_will_be_visible_on_all_slides +'</div></td>';
  switch(type) {
    case 'text': {
...
function wds_add_slide() {
  ...
  if (spider_uploader) {
    ...
    update_thumb_by = ' <input type="button" class="action_buttons edit_thumb wds_free_button" id="button_image_url' + slideID + '" onclick="alert(\''+ wds_object.translate.this_functionality_is_disabled_in_free_version +'\'); return false;" value="Edit Thumbnail" />';
    ...
    img_layer_upload_by = ' <a href="' + (fv ? uploader_href_for_add_layer : "") + '" class="' + (" wds_free_button") + ' action_buttons add_image_layer button-small" title="'+ wds_object.translate.add_image_layer +'" onclick="' + ("alert('"+ wds_object.translate.this_functionality_is_disabled_in_free_version +"')") + '; return false;">'+ wds_object.translate.add_image_layer +'</a>';
  }
  else {
    slide_upload_by = ' <input id="button_image_url' + slideID + '" class="action_buttons edit_slide" type="button" value="'+ wds_object.translate.add_edit_image +'" onclick="spider_media_uploader(\'' + slideID + '\', event); return false;" />';
    edit_slide_by = ' <span class="wds_change_thumbnail" type="button" value="'+ wds_object.translate.add_edit_image +'" onclick="spider_media_uploader(\'' + slideID + '\', event); return false;" ></span>';
     update_thumb_by = ' <input type="button" class="action_buttons edit_thumb wds_free_button" id="button_image_url' + slideID + '" onclick="alert(\''+wds_object.translate.this_functionality_is_disabled_in_free_version +'\'); return false;" value="'+ wds_object.translate.edit_thumbnail +'" />';
    img_layer_upload_by = ' <input class="action_buttons add_image_layer ' + (" wds_free_button") + '  button-small" type="button" value="'+ wds_object.translate.add_image_layer +'" onclick="' + ("alert('"+ wds_object.translate.this_functionality_is_disabled_in_free_version +"')") + '; return false;" />';
  }
    ...
    '<div id="wds_subtab_wrap' + slideID + '" class="wds_subtab_wrap connectedSortable"><div id="wbs_subtab' + slideID + '" class="tab_link wds_sub_active" href="#" style="display:block !important; width:149px; height:140px; padding:0; margin-right: 25px;">' +
      '<div  class="tab_image" id="wds_tab_image' + slideID + '">' + 
        '<div class="tab_buttons">' + 
          '<div class="handle_wrap"><div class="handle" title="'+ wds_object.translate.drag_to_re_order +'"></div></div>' +
  ...
  jQuery(".wbs_subtab").after(
    '<div class="wds_box wds_sub_active wds_slide' + slideID + '">' +
      '<table class="ui-sortable' + slideID + '">' +
        ...
        '<tbody>' +
          '<input type="hidden" name="type' + slideID + '" id="type' + slideID + '" value="'+ wds_object.translate.image +'" />' +
          '<tr class="bgcolor"><td colspan="4"><div class="slide_add_buttons_wrap">' +
            slide_upload_by +
            '</div><div class="slide_add_buttons_wrap"><input class="action_buttons add_by_url" type="button" value="'+ wds_object.translate.add_image_by_url+'" onclick="wds_add_image_url(\'' + slideID + '\')" />' +
	    '</div><div class="slide_add_buttons_wrap"><input type="button" class="action_buttons add_video wds_free_button" onclick="alert(\''+ wds_object.translate.this_functionality_is_disabled_in_free_version +'\')"" value="'+ wds_object.translate.add_video +'" />' +
            '</div><div class="slide_add_buttons_wrap"><input class="action_buttons embed_media wds_free_button" type="button" value="'+ wds_object.translate.embed_media +'" onclick="alert(\''+ wds_object.translate.this_functionality_is_disabled_in_free_version +'\')">' +
	    '</div><div class="slide_add_buttons_wrap"><input class="action_buttons add_post wds_free_button" type="button" value="'+ wds_object.translate.add_post +'" onclick="alert(\''+ wds_object.translate.this_functionality_is_disabled_in_free_version +'\')">' +
            '</div><div class="slide_add_buttons_wrap"><input id="delete_image_url' + slideID + '" class="action_buttons delete" type="button" value="'+ wds_object.translate.delete +'" onclick="wds_remove_slide(\'' + slideID + '\')" />' + update_thumb_by + 
          ...
          '</tr><tr><td><table class="wds_slide_radio_left"><tr><td class="spider_label"><label>'+ wds_object.translate.published +'</label></td>' +
              '<td><input id="published' + slideID + '1" type="radio" value="1" checked="checked" name="published' + slideID + '">' +
                  '<label for="published' + slideID + '1">'+ wds_object.translate.yes +'</label>' +
                  '<input id="published' + slideID + '0" type="radio" value="0" name="published' + slideID + '">' +
                  '<label for="published' + slideID + '0">'+ wds_object.translate.no +'</label></td>' +
          '</tr></table><table class="wds_slide_radio_right"><tr id="trlink' + slideID + '"><td  title="'+ wds_object.translate.you_can_set_a_redirection_link_for_external_links +'" class="wds_tooltip spider_label"><label for="link' + slideID + '">'+ wds_object.translate.link_the_slide_to +'</label></td>' +
                   '<td><input id="link' + slideID + '" type="text"  value="" name="link' + slideID + '" />' +
                       '<input id="target_attr_slide' + slideID + '" type="checkbox"  name="target_attr_slide' + slideID + '" value="1" checked="checked" /><label for="target_attr_slide' + slideID + '">'+ wds_object.translate.open_in_a_new_window +'</label>' +
                       '</td>' +
          '</tr></table></td></tr><tr class="bgcolor"><td colspan="4">' +
            '<div class="layer_add_buttons_wrap"><input class="action_buttons add_text_layer ' + (!fv ? "" : " wds_free_button") + '  button-small" type="button" value="'+ wds_object.translate.add_text_layer +'" onclick="' + (!fv ? "wds_add_layer(\'text\', \'" + slideID + "\')" : "alert('"+ wds_object.translate.this_functionality_is_disabled_in_free_version +"')") + '; return false;">' +
            img_layer_upload_by +
	    '</div><div class="layer_add_buttons_wrap"><input class="action_buttons add_video_layer button-small wds_free_button" type="button" onclick="alert(\''+ wds_object.translate.this_functionality_is_disabled_in_free_version +'\'); return false;" value="'+ wds_object.translate.add_video_layer +'" />' +
            '</div><div class="layer_add_buttons_wrap"><input class="action_buttons add_embed_layer button-small wds_free_button" type="button" value="'+ wds_object.translate.embed_media_layer +'" onclick="alert(\''+ wds_object.translate.this_functionality_is_disabled_in_free_version +'\'); return false;" />' +
            ' </div><div class="layer_add_buttons_wrap"><input class="action_buttons add_social_layer button-small wds_free_button" type="button" value="'+wds_object.translate.add_social_buttons_layer +'" onclick="alert(\''+ wds_object.translate.this_functionality_is_disabled_in_free_version +'\'); return false;">' +
	    '</div><div class="layer_add_buttons_wrap"><input class="action_buttons add_hotspot_layer button-small wds_free_button" type="button" value="'+wds_object.translate.add_hotspot_layer +'" onclick="alert(\''+ wds_object.translate.this_functionality_is_disabled_in_free_version +'\'); return false;"></td>' +
          '</tr></tbody></table>' +
...
function wds_check_number() {
  ...
  if (number != '' && (number < 0 || number > 1)) {
      alert(wds_object.translate.the_thumbnail_size_must_be_between_0_to_1);
...
function add_new_callback(par_tr, select) {
	var select_val = select.val(),
      ...
      textarea_html = "";
  par_tr.next().find("td").append("<div class='callbeck-item'><span class='spider_label_options'>" + selected.text() + "</span><textarea class='callbeck-textarea' name='" + select_val + "'>" + textarea_html + "</textarea><button type='button' id='remove_callback' class='action_buttons remove_callback' onclick=\"remove_callback_item(this);\">"+ wds_object.translate.remove +"</button></div>");
...
function wds_bulk_actions(that) {
  ...
  if (action == 'export') {
    alert(wds_object.translate.this_functionality_is_disabled_in_free_version);
  }
