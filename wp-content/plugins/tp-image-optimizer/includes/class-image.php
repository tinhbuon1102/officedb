...
if (!class_exists('TP_Image_Optimizer_Image')) {
    ...
    class TP_Image_Optimizer_Image {
                ...
                if (count($attachment_ids) != 0) {
                    foreach ($attachment_ids as $attachment_id) {
                        ...
                        if (wp_attachment_is_image($attachment_id)) {
                            $stastics     = $db_table->get_all_stastic_image($attachment_id, "full");
                            $origin_size  = tp_image_optimizer_caculator_size($stastics[0]['origin_size']);
                            $current_size = tp_image_optimizer_caculator_size($stastics[0]['current_size']);
                            // Remove comma
                            ...
                            if ($image_status == 'optimized') {
                                ...
                                $action = "<span class='result compress-$attachment_id' data-id='$attachment_id'><a href='$attachment_id' class='single-compress button button-secondary'><b>" . esc_html__("Compress", 'tp-image-optimizer') . "</b></a><p class='spinner'></p></span>";
                            }
                            ...
                            $item      = array(
                                ...
                                'optimizer'    => $this->get_reduce_after_optimized($attachment_id, $origin_size, $current_size),
                                'detail'       => "<a href='$attachment_id' class='badge button button-secondary' id='badge-$attachment_id' ><b>" . esc_html__("View", 'tp-image-optimizer') . "</b></a>$action",
                            );
        ...
        public function count_attachment_file() {
            $count = get_transient('tp_image_optimizer_images_count');

            if (empty($count)) {
                $counts = wp_count_attachments(array('image'));
                $count  = array_sum((array) $counts);
                set_transient('tp_image_optimizer_images_count', $count);
            }
        ...
        public static function remove_attachment_count() {
            delete_transient('tp_image_optimizer_images_count');
        }
