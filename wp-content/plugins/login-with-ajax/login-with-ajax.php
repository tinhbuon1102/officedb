...
class LoginWithAjax {
	...
	public static function register(){
	    ...
	    elseif ( $_POST['passw1'] !== $_POST['passw2'] ) {
	    	$errors = new WP_Error();
	    	$errors->add( 'passwords_not_matched', __("<strong>ERROR</strong>: Passwords not matched"), 'realty' );
	    }
	    elseif (!$_POST['passw1'])
	    {
	    	$errors = new WP_Error();
	    	$errors->add( 'passwords_empty', __("<strong>ERROR</strong>: Passwords is empty"), 'realty' );
	    }
		...
		if(is_user_logged_in()){
		    ...
		    if(  defined('ICL_LANGUAGE_CODE') ){
		        if( !function_exists('lwa_wpml_input_var') ){
                    function lwa_wpml_input_var(){ echo '<input type="text" name="lang" id="lang" value="'.esc_attr(ICL_LANGUAGE_CODE).'" />'; }
		        }
