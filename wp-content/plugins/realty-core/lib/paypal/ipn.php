...
if( $paypal_settings_sandbox ) {
	...
	$listener->use_ssl = true; // Sandbox requires SSL
}
  ...
  // https://developer.paypal.com/webapps/developer/docs/classic/ipn/integration-guide/IPNIntro/
	...
	if ( $_POST['payment_status'] == "Completed" && $_POST['receiver_email'] == $paypal_settings_merchant_id ) {
  	...
  	$property_id = intval( $_POST['item_number'] );
		...
		if ( get_post_type( $property_id ) == 'property' ) { // Pay per listing
		...
		} else { // Create invoice for membership package
		...
		if ( isset( $_POST['custom'] ) ) {
			$user_id = $_POST['custom']; // Our custom PayPal button parameter for passing "user_id"
			tt_create_user_invoice( $user_id, $property_id, 'PayPal' );
		}
