            ...
            if ( !empty( $this->field['wbc_demo_imports'] ) ) {
                ...
                foreach ( $this->field['wbc_demo_imports'] as $section => $imports ) {
                    ...
                    if ( !array_key_exists( 'imported', $imports ) ) {
                        ...
                        $imported = false;
                        $import_message = esc_html__( 'Import Demo', 'realty' );
                    }else {
                        $imported = true;
                        $extra_class = 'active imported';
                        $import_message = esc_html__( 'Demo Imported', 'realty' );
                    }
                    ...
                    if ( $imports['directory'] == 'demo-full' ) {
	                    $demo_title = esc_html__( 'Demo - Full', 'realty');
                    }
                    ...
                    echo '<h3 class="theme-name">' . $demo_title . '</h3>';

                    echo '<div class="theme-actions">';
                    if ( false == $imported ) {
                        echo '<div class="wbc-importer-buttons"><span class="spinner">'.esc_html__( 'Please Wait...', 'realty' ).'</span><span class="button-primary importer-button import-demo-data">' . esc_html__( 'Import Demo', 'realty' ) . '</span></div>';
                    }else {
                        echo '<div class="wbc-importer-buttons button-secondary importer-button">'.esc_html__( 'Imported', 'realty' ).'</div>';
                        echo '<span class="spinner">'.esc_html__( 'Please Wait...', 'realty' ).'</span>';
                        echo '<div id="wbc-importer-reimport" class="wbc-importer-buttons button-primary import-demo-data importer-button">'.esc_html__( 'Re-Import', 'realty' ).'</div>';
                    }
                    echo '</div>';
                    echo '</div>';
                ...
                echo "<h5>".esc_html__( 'No Demo Data Provided', 'realty' )."</h5>";
            }
