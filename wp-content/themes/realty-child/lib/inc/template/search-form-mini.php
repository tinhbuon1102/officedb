...
<form class="property-search-form border-box" action="<?php if ( tt_page_id_template_search() ) { echo get_permalink( tt_page_id_template_search() ); } ?>">
	...
	<div class="row search-form-row">
		...
		<?php
			...
			// Don't show "Features"
			$property_search_features = false;
			...
			} else if ( isset( $realty_theme_option['property-search-mini-parameter'] ) && ! empty( $realty_theme_option['property-search-mini-parameter'] ) ) {
			...
			} else if ( isset( $realty_theme_option['property-search-mini-label'] ) && ! empty( $realty_theme_option['property-search-mini-label'] ) ) {
									...
									<?php if ( $realty_theme_option['property-search-price-dropdown'] == 'dropdown-range' ) { ?>
										...
										<select name="<?php echo $search_parameter; ?>" id="<?php echo $search_parameter; ?>" class="<?php echo esc_attr( $form_select_class ); ?>">
											...
											<?php while ( $step_min <= $pricerange_max ) { ?>
												...
												<option value="<?php echo $step_min; ?>" <?php selected( isset( $_GET[$search_parameter] ) ? $_GET[$search_parameter] : null, $step_min ); ?>><?php echo $step_min . '+'; ?></option>
												<?php } else { ?>
												<option value="<?php echo $step_min . '-' . $step_max; ?>" <?php selected( isset( $_GET[$search_parameter] ) ? $_GET[$search_parameter] : null, $step_min . '-' . $step_max ); ?>><?php echo $step_min . ' - ' . $step_max; ?></option>
												<?php } ?>
			...
			<?php } else { ?>
				...
				<div class="col-xs-12" style="margin-bottom: 20px">
					<p class="alert alert-info">
						...
						<?php esc_html_e( 'Please setup "Property "Search Mini" under "Appearance > Theme Options > Property Search".', 'realty' ); ?>
